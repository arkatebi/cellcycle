...
def set_parameters(network):
    ...
    if (arg_dict['knockdownlist']):  
        knock_out_list = arg_dict['knockdownlist'][0].split(",")
    else: 
        knock_out_list = arg_dict['knockdownlist']
...
#-----------------------------------------------------------------------------#
def open_parameter_files(network): 
    ''' 
    ...
    fname_dict_nodeparams=network.get_fname_nodeparams()
    fname_dict_edgeparams=network.get_fname_edgeparams()
...
#-----------------------------------------------------------------------------#
def open_solution_files(network): 
    ''' 
    This method opens output files for writing solutions. 
    It creats MAX_STABLE_STATES number of file handles in the write mode, 
    places in a dictionary, and returns the dictionary. 
    '''
    save_path=network.get_work_dir()
    #print('Saving files in the folder: '+save_path)
    config_dict=network.get_config_dict()
    tpo_fname=network.get_tpo_fname()
...
#----------------------------------------------------------------------#
def generate_expressions(network): 
    '''
